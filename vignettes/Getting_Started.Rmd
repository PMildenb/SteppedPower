...
* subject specific random intercepts (closed cohort settings)
* random time trends
* cluster- / timepoint-specific residual variances

with leads to the following extended model formula:

$$y_{ijk}= \mu_0 + \alpha_i + X (\theta_{ij} + c_j) + b_j + d_{ik} + t_{ij} + e_{ijk}$$ 
with  

* $c_j$ a random treatment effect
* $b_j$ and $c_j$ jointly distributed with $\left(\begin{smallmatrix} b_j \\ c_j \end{smallmatrix} \right) \sim N\left(\begin{smallmatrix} \tau^2 & \rho\tau\eta \\ \rho\tau\eta & \eta^2\end{smallmatrix}\right)$
* $d_{ik}$ is a subject specific random intercept (constant over time), normally
distributed with $d_{ik}\sim N(0,\psi^2)$
* $t_{ij}$ a random time effect with $t_{ij}\sim N(0,\gamma^2)$
* $e_{ijk}\sim N(0,\sigma_{ij})$
