...
identity link are implemented (see also the [Binomial Outcomes](Binomial_Outcomes.html) vignette).
...
Incomplete designs with unobserved transition periods (on the right) are defined with the `trt_Delay` argument. Designs with unobserved *"edge periods"* are defined with the `incomplete` argument. This is described in more detail in the [Incomplete Designs](Incomplete_Designs.html) vignette.
...
`SteppedPower` can also handle binomial and count outcomes (see the [Binomial Outcomes](Binomial_Outcomes.html) vignette for details).
