...
indentity link are implemented. 
...
A different approach to establish a covariance structure is to directly assume cluster cell correlation as proposed by \cite{Preisser2003integrated} and introduced to the context of stepped wedge designs in \cite{li2018sample}. They use $\boldsymbol{\alpha}:=(\alpha_0, \alpha_1, \alpha_2)$ where 
...
Open1 <- wlsPower(mu0=0, mu1=5, Cl=rep(3,3), 
                  sigma=5, tau=1, psi=3, AR=c(1,1,.75),
                  N=3, verbose=2)
...
and autoregressive (on individual level). Let's have a look on a toy example with
three cluster, each consisting of three individuals. The covariance matrix could
be visualised as follows:

```{r}
Open2Indiv <- wlsPower(mu0=0, mu1=10, Cl=c(1,1,1,0), 
                       sigma=1, tau=5, psi=10, AR=c(1,1,.65),
                       N=3, verbose=2, INDIV_LVL=TRUE)

covar <- as.matrix(Open2Indiv$CovarianceMatrix)
seqLength <- seq_len(dim(covar)[1])
p <- plotly::plot_ly(type="heatmap", colors=c("white","steelblue"),
             x=~seqLength, y=~seqLength, z=~covar, xgap=1, ygap=1, showscale=FALSE)
plotly::layout(p,
...
covariance matices, most notably [@kasza2020sample] and [@li2020design]. 
...
### Closed formula for proportional decay stucture
...
or churn rate `chi`. It can be understood as the estimated probability 
