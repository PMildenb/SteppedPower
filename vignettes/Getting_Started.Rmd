...
plot(wlsPwr,which=1)$WgtPlot # (Actively accessing the first list element is needed to trigger plotting in an rmarkdown-environment) 
```

The information content, as described in [@kasza2019information], looks like this:

```{r}
plot(wlsPwr,which=2)$ICplot
```
...
Closed2
plot(Closed2, annotations=FALSE)$WgtPlot
```
...
plot(Open2Indiv, which=4)$CMplot
```
